// 完整注册流程集成测试 - HTTP/WebSocket/Storage 集成测试
// 测试 OTP 请求、验证、用户注册的完整流程，包括 HTTP API 和 WebSocket 通信
...
#[tokio::test]
async fn test_registration_via_http_api() {
    use axum::{
    ...
    io.ns("/", move |socket: SocketRef| {
        ...
        socket.on("subscribe", move |socket: SocketRef, Data(collection): Data<String>| {
            let _ = socket.join(collection.clone());
            tracing::info!("Socket {} subscribed to {}", socket.id, collection);
        });
    });

    // 创建 HTTP 路由
    let app = Router::new()
        .route("/collections/:collection", post(create_http_doc))
        .route("/collections/:collection/:id", get(get_http_doc))
        .route("/collections/:collection/:id", put(update_http_doc))
        .route("/query", post(query_http_docs))
    ...
    // 步骤 1: 创建 OTP 记录 (模拟 Hook 操作)
    let email = "http_test@example.com";
    let otp = "999888";
    let otp_record = Document::new(
        "_internal_otps".to_string(),
        json!({
    ...
    let otp_response = client
        .post(format!("{}/collections/_internal_otps", base_url))
        .json(&otp_record.data)
        .send()
        .await
        .unwrap();

    assert_eq!(otp_response.status(), reqwest::StatusCode::OK);
    let created_otp: Document = otp_response.json().await.unwrap();

    // 步骤 2: 验证 OTP 创建成功
    let get_response = client
        .get(format!("{}/collections/_internal_otps/{}", base_url, created_otp.id))
        .send()
        .await
        .unwrap();

    assert_eq!(get_response.status(), reqwest::StatusCode::OK);
    let retrieved_otp: Document = get_response.json().await.unwrap();
    assert_eq!(retrieved_otp.data["email"], email);
    assert_eq!(retrieved_otp.data["otp"], otp);
    ...
    assert_eq!(user_response.status(), reqwest::StatusCode::OK);
    let created_user: Document = user_response.json().await.unwrap();

    // 步骤 4: 验证用户创建成功
    let get_user_response = client
        .get(format!("{}/collections/users/{}", base_url, created_user.id))
        .send()
        .await
        .unwrap();

    assert_eq!(get_user_response.status(), reqwest::StatusCode::OK);
    let retrieved_user: Document = get_user_response.json().await.unwrap();
    assert_eq!(retrieved_user.data["email"], email);
    assert_eq!(retrieved_user.data["status"], "active");

    // 步骤 5: 更新 OTP 为已使用 (保留原始字段)
    ...
    let update_response = client
        .put(format!("{}/collections/_internal_otps/{}", base_url, created_otp.id))
        .json(&update_data)
        .send()
        .await
        .unwrap();
    ...
    let query_results: Vec<Document> = query_response.json().await.unwrap();

    // 调试信息：如果查询失败，打印详细信息
    if query_results.len() != 1 {
        eprintln!("Expected 1 result, got {}", query_results.len());
        eprintln!("Query was looking for email: {} and used: true", email);
        ...
        if all_response.status().is_success() {
            if let Ok(all_otp) = all_response.json::<Vec<Document>>().await {
                eprintln!("Total OTP records in database: {}", all_otp.len());
                for (i, otp) in all_otp.iter().enumerate() {
                    eprintln!("OTP {}: data={}", i, otp.data);
                }
            }
        }
    }
    ...
    assert_eq!(query_results[0].data["used"], true);

    // 清理
    server_task.abort();
    println!("✓ HTTP API registration test completed successfully");
}

// HTTP 处理函数辅助函数
async fn create_http_doc(
...
async fn update_http_doc(
    State(state): State<Arc<AppState>>,
    Path((collection, id)): Path<(String, String)>,
    Json(data): Json<serde_json::Value>,
) -> Json<Document> {
    let mut doc = match state.storage.get(&collection, &id).await.unwrap() {
        Some(mut d) => {
            d.data = data;
            d.version += 1;
...
async fn query_http_docs(
    State(state): State<Arc<AppState>>,
    Json(query): Json<serde_json::Value>,
) -> Json<Vec<Document>> {
    // 简化的查询解析
    let collection = query["collection"].as_str().unwrap().to_string();
    let filters_value = query["filters"].as_array().unwrap();

    let mut filters = Vec::new();
    for filter in filters_value {
        if let Some(arr) = filter.as_array() {
            if arr.len() == 2 {
                let field = arr[0].as_str().unwrap().to_string();
                let op_value = &arr[1];

                if let Some(obj) = op_value.as_object() {
                    if let Some(eq_val) = obj.get("Eq") {
                        filters.push((field, QueryOp::Eq(eq_val.clone())));
                    }
                }
            }
        }
    ...
    let query_obj = Query {
        collection,
        filters,
        limit: query["limit"].as_u64().map(|u| u as usize),
        offset: query["offset"].as_u64().map(|u| u as usize),
    };

    let docs = state.storage.query(query_obj).await.unwrap();
    Json(docs)
}
...
#[tokio::test]
async fn test_registration_with_websocket_notifications() {
    use socketioxide::{SocketIo, extract::SocketRef, extract::Data};
    ...
    io.ns("/", move |socket: SocketRef| {
        let st = Arc::clone(&state_ns);
        let notif_clone = notification_received_clone.clone();
        ...
        socket.on("register_notification", move |socket: SocketRef, Data(_data): Data<serde_json::Value>| {
            notif_clone.store(true, Ordering::SeqCst);
            tracing::info!("Registration notification received");
        });
    });
    ...
    let app = Router::new()
        .route("/trigger", post(trigger_registration))
        .layer(CorsLayer::permissive())
        .layer(io_layer)
        .with_state(state.clone());
    ...
    // 验证通知被接收 (这里简化测试，实际需要 WebSocket 客户端)
    println!("✓ WebSocket notification test completed");
}
...
async fn trigger_registration(
    ...
    let otp = data["otp"].as_str().unwrap();
    ...
    let _ = state.storage.insert(otp_record).await;

    // 发送事件
    let _ = state.event_bus.emit(Event {
        event_type: EventType::VerificationCodeRequested,
        payload: json!({"email": email, "status": "sent"}),
        timestamp: chrono::Utc::now().timestamp_millis(),
    });
    ...
    let user_id = user.id.clone();
    let _ = state.storage.insert(user).await;

    // 发送用户创建事件
    let _ = state.event_bus.emit(Event {
        event_type: EventType::UserCreated,
        payload: json!({"user_id": user_id, "email": email}),
        timestamp: chrono::Utc::now().timestamp_millis(),
    });
